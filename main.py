...
def parse_args(args=None):
    ...
    # add full imagenet
    parser.add_argument("--dataset", default="c10", type=str, help="[c10, c100, imagenette, svhn, imagenet]")
    parser.add_argument("--data-dir", default=None, type=str, help="Directory to download/store ImageNet dataset")
    ...
    parser.add_argument("--rcpaste", default=False, action="store_true")
    ...
    parser.add_argument("--attn-type", default="softmax")
    parser.add_argument("--train-corruption")
    ...
    if args.attn_type == "sigmoid":
        print("[INFO] using SIGMOID attn")
        args.attn = MultiHeadSigmoidSelfAttention
    elif args.attn_type == "linear":
        print("[INFO] using LINEAR attn")
        args.attn = MultiHeadLinearSelfAttention
    elif args.attn_type == "doublystochastic":
        print("[INFO] using DOUBLY STOCHASTIC attn")
        args.attn = MultiHeadDoublyStochasticSelfAttention
    elif args.attn_type == "cosine":
        print("[INFO] using COSINE attn")
        args.attn = MultiHeadCosineSelfAttention
    else:
        print("[INFO] using SOFTMAX attn")
        args.attn = MultiHeadSoftmaxSelfAttention
    return args
...
if __name__ == "__main__":
    ...
    net = Net(args)
    if args.grad_clip:
        trainer = pl.Trainer(accelerator='gpu', precision=args.precision,fast_dev_run=args.dry_run, benchmark=args.benchmark, logger=logger, max_epochs=args.max_epochs, gradient_clip_val=args.grad_clip)
    else:
        trainer = pl.Trainer(accelerator='gpu', precision=args.precision,fast_dev_run=args.dry_run, benchmark=args.benchmark, logger=logger, max_epochs=args.max_epochs)
