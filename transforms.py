...
def gaussian_noise(x, severity=1):
    c = [0.04, 0.06, .08, .09, .10, .12, .14, .16, .18, .20][severity - 1]
...
def defocus_blur(x, severity=1):
    ...
    # x = x.transpose((2, 0, 1)) # from 32 32 3 to 3 32 32
...
class CIFAR_Corruption:
    def __init__(self, name, severity=1):
        self.corruption = globals()[name]
        self.name = name

    def __call__(self, pic):
        return self.corruption(pic)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default="c10", type=str, help="[c10, c100, imagenette]")
    parser.add_argument("--severity-min", default="1", type=int)
    parser.add_argument("--severity-max", default="4", type=int)
    corruption_names = """
    gaussian-noise
    shot-noise
    impulse-noise
    ...
    saturate
    """
    parser.add_argument("--corruption", default="gaussian-noise", type=str, help="["+corruption_names+"]")
    parser.add_argument("--regenerate", default=False, type=int)
    parser.add_argument("--autoaugment", default=False, type=int)
    ...
    if dataset == 'c10':
        train_data = dset.CIFAR10('data', train=True, download=True)
        test_data = dset.CIFAR10('data', train=False, download=True)
    elif dataset == 'c100':
        train_data = dset.CIFAR100('data', train=True, download=True)
        test_data = dset.CIFAR100('data', train=False, download=True)
        ...
        raise Exception("dataset provided was not c10, c100 or imagenette")
    ...
    if dataset == "c10" or dataset == "c100":
        ...
        os.makedirs('data/'+dataset+'-C', exist_ok=True)
        np.save('data/'+dataset+'-C/labels_train.npy',
                np.array(labels_train).astype(np.uint8))
        np.save('data/'+dataset+'-C/labels_test.npy',
                np.array(labels_test).astype(np.uint8))
    elif dataset == "imagenette":
        for img_path, label in train_data.imgs:
            labels_train.append(label)
        for img_path, label in test_data.imgs:
            labels_test.append(label)
        os.makedirs('data/imagenette-C', exist_ok=True)
        np.save('data/'+dataset+'-C/labels_train.npy',
                np.array(labels_train))
        np.save('data/'+dataset+'-C/labels_test.npy',
                np.array(labels_test))
    
    corruption_method = d[args.corruption]
    print('Creating images for the corruption', corruption_method.__name__)
    
    for severity in range(args.severity_min,args.severity_max):
        if not regenerate and os.path.exists('data/'+dataset+'-C/' + corruption_method.__name__ + f'_{severity}_test.npy'):
            print('Already created, not regenerating!')
            continue
        if dataset == "c10" or dataset == "c100":
            cifar_c_train = []
            cifar_c_test = []
            np.random.seed(0)
            corruption = lambda clean_img: corruption_method(clean_img, severity)
            
            for img, label in zip(train_data.data, train_data.targets):
                labels_train.append(label)
                cifar_c_train.append(np.uint8(corruption(convert_img(img))))

            for img, label in zip(test_data.data, test_data.targets):
                labels_test.append(label)
                cifar_c_test.append(np.uint8(corruption(convert_img(img))))

            np.save('data/'+dataset+'-C/' + corruption_method.__name__ + f'_{severity}_train.npy',
                    np.array(cifar_c_train).astype(np.uint8))
        
            np.save('data/'+dataset+'-C/' + corruption_method.__name__ + f'_{severity}_test.npy',
                    np.array(cifar_c_test).astype(np.uint8))
        elif dataset == "imagenette":
            corrupted_train = []
            corrupted_test = []
            ...
            resize = trn.Resize((224,224)) # resize so all images are same size and can save as .npy
            # Process training set
            for img_path, label in train_data.imgs:
                # Load image
                img = Image.open(img_path).convert('RGB')
                img = resize(img)
                # Convert to numpy array since convert_img function expects numpy
                img_array = np.array(img)
                # Apply corruption
                corrupted = np.uint8(corruption(convert_img(img_array)))
                
                corrupted_train.append(corrupted)
                labels_train.append(label)
            ...
            np.save('data/'+dataset+'-C/' + corruption_method.__name__ + f'_{severity}_train.npy',
                    np.array(corrupted_train).astype(np.uint8))
            np.save('data/'+dataset+'-C/' + corruption_method.__name__ + f'_{severity}_test.npy',
                np.array(corrupted_test).astype(np.uint8))
